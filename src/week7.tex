...
\[
    d\vb{n}_p\colon T_p S\to T_{\vb{n}_p}\mathbb{S}^2    .
\]
\(\forall v\in T_p S\), let \(\alpha(s)\) be the curve on \(S\)
such that \(\alpha(0)=p\), \(\alpha'(0)=v\).
\[
    \Rightarrow
    d\vb{n}_p(v)=\left.\dv{s}\right|_{s=0}\vb{n}\left(\alpha(s)\right)
...
We have seen that \(d \vb{n}_p\colon T_p S\to T_{\vb{n}_p}\mathbb{S}^2\)
has its image actually lying in \(T_pS=\Span\left\{\varphi_u,
...
\end{remark}
\section{Geometric meaning of the \engordnumber{2} fundamental form and curvatures on a regular surface}
...
\subsection{Normal curvature}
    ...
    \[
        ...
        }\alpha(s),\quad \vb{n}:\text{unit normal vector of the curve }\alpha(s)
    \]
\begin{align*}
...
\end{itemize}
\begin{example}Sphere: \(x^2+y^2+z^2=1\).
    \label{normal curvature-sphere}
\end{example}
...
\newcommand\DrawLatitude[3][\AngleGamma]{
  ...
  \draw[current plane,thin,dashed] (180-\angVis:1) arc (180-\angVis:\angVis:1);
            ...
            \end{tikzpicture}
        \end{center}
     \begin{example}Cylinder: \(x^2+y^2=1\).
        \label{normal curvature-cylinder}
     \end{example}
        ...
        \begin{center}
            ...
            \end{tikzpicture}
        \end{center}
        \begin{example}Catenoid: \(\varphi(u,v)=
            \left(c\cosh\frac{u}{c}\cos v,
            c\cosh\frac{u}{c}\sin v,\sinh u\right)\)
            \label{normal curvature-catenoid}
        \end{example}
        ...
        \begin{tikzpicture}
            \begin{axis}[xticklabels={,,},%不显示x坐标轴数字
                ...
                \addplot3 [
                    ...
                    samples=40,
                    samples y=1
                ]({cosh(t)*-0.96592582628},{cosh(t)*-0.2588190451},{sinh(t)});
